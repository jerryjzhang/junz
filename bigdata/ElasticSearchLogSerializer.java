...
public class ElasticSearchLogSerializer implements ElasticSearchEventSerializer{
	 private static final Pattern PATTERN = Pattern.compile("(\\d+:\\d+:\\d+.\\d+) \\[(.*)]\\s+(\\S+)\\s+(\\S+) (.*)");
	 ...
	 public XContentBuilder getContentBuilder(Event event) throws IOException {
		 ...
		 if(m.find()){
			 ...
			 if(timestamp != null){
				 //ContentBuilderUtil.appendField(builder, "timestamp", ddf.format(timestamp).getBytes());
				 builder.field("@timestamp", timestamp);
			 }
			 ContentBuilderUtil.appendField(builder, "thread", m.group(2).getBytes());
			 ContentBuilderUtil.appendField(builder, "level", m.group(3).getBytes());
			 ContentBuilderUtil.appendField(builder, "logger", m.group(4).getBytes());
			 ContentBuilderUtil.appendField(builder, "message", m.group(5).getBytes());
		 }
